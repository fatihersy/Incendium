...
static sound_system_state *restrict state;
...
bool sound_system_on_event(u16 code, event_context context) {
  ...
  case EVENT_CODE_PLAY_SOUND:{
    play_sound(context.data.i32[0]);
    return true;
  }
  case EVENT_CODE_PLAY_MUSIC:{
    play_music(context.data.i32[0]);
    return true;
  }
  case EVENT_CODE_RESET_SOUND:{
    reset_sound(context.data.i32[0]);
    return true;
  }
  case EVENT_CODE_RESET_MUSIC:{
    reset_music(context.data.i32[0]);
    return true;
  }
