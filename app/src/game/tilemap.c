  ...
  for (u16 y = start_y; y < end_y; ++y) {
    for (u16 x = start_x; x < end_x; ++x) {
      ...
      for (int i = 0; i < MAX_TILEMAP_LAYERS; ++i) {
        render_tile(&_tilemap->tiles[i][x][y], (Rectangle) { x_pos, y_pos, _tilemap->tile_size, _tilemap->tile_size}, sheet);
      }
    }
  }
...
void render_tilesheet(tilesheet* sheet, f32 zoom) {
  ...
  for (u16 i = 0; i < sheet->tile_count; ++i) {
    ...
    tile_symbol _tile = (tile_symbol) { .c[0] = x + TILEMAP_TILE_START_SYMBOL, .c[1] = y + TILEMAP_TILE_START_SYMBOL };

    render_tile(&_tile, (Rectangle) {x_pos, y_pos, sheet->dest_tile_size * zoom, sheet->dest_tile_size * zoom}, sheet);
  }
}
/**
 * @brief Unsafe!
 ...
 */
inline void render_tile(tile_symbol* symbol, Rectangle dest, tilesheet* sheet) {
  u16 x = (symbol->c[0] - TILEMAP_TILE_START_SYMBOL) * sheet->tile_size;
  u16 y = (symbol->c[1] - TILEMAP_TILE_START_SYMBOL) * sheet->tile_size;
  DrawTexturePro(*sheet->atlas_handle, (Rectangle) { x, y, sheet->tile_size, sheet->tile_size}, dest, (Vector2){0},0.f, WHITE);
}
...
void str_to_map(tilemap* map, tilemap_stringtify_package* out_package) {
  ...
  for (int i = 0; i < MAX_TILEMAP_PROPS; ++i) {
    ...
    prop->source = (Rectangle) {
      str_to_F32((const char*)&str_par.buffer[2]), str_to_F32((const char*)&str_par.buffer[3]),
      str_to_F32((const char*)&str_par.buffer[4]), str_to_F32((const char*)&str_par.buffer[5]),
    };
    prop->dest = (Rectangle) {
      str_to_F32((const char*)&str_par.buffer[6]), str_to_F32((const char*)&str_par.buffer[7]),
      str_to_F32((const char*)&str_par.buffer[8]), str_to_F32((const char*)&str_par.buffer[9]),
    };
