...
fshader *get_shader_by_enum(shader_id _id) {
  if (_id >= SHADER_ID_MAX || _id <= SHADER_ID_UNSPECIFIED) {
    TraceLog(LOG_WARNING, "resource::get_shader_by_enum()::Shader type out of bound");
    return nullptr;
  }
...
void set_shader_uniform(shader_id _id, i32 index, data128 _data_pack) {
  ...
  case SHADER_UNIFORM_FLOAT: {
    ...
    SetShaderValue(
      ...
      state->shaders[_id].locations[index].data.f32,
      data_id
    );
    break;
  }
  ...
  case SHADER_UNIFORM_INT: {
    shader->locations[index].data = _data_pack;
    SetShaderValue(state->shaders[_id].handle, index,
                   &state->shaders[_id].locations[index].data.i32[0],
                   data_id);
    break;
  }
  case SHADER_UNIFORM_IVEC2: {
    shader->locations[index].data = _data_pack;
    SetShaderValue(state->shaders[_id].handle, index,
                   &state->shaders[_id].locations[index].data.i32,
                   data_id);
    break;
  }
  ...
  case SHADER_UNIFORM_SAMPLER2D: {
    ...
    SetShaderValue(
      state->shaders[_id].handle, 
      index, 
      state->shaders[_id].locations[index].data.address,
      data_id
    );
    break;
  }
