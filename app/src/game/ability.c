...
void add_ability(ability_system_state* system, ability_type type) {
    ...
    switch (type) {
        case fireball: {
            _ability->projectiles = (Character2D*)malloc(FIRE_BALL_BALL_COUNT * sizeof(Character2D));
        ...
        case salvo: {
            _ability->projectiles = (Character2D*)malloc(SALVO_PROJECTILE_COUNT * sizeof(Character2D));
            _ability->projectile_process = (float*)malloc(SALVO_PROJECTILE_COUNT * sizeof(float));
            _ability->projectile_target_position = (Vector2*)malloc(SALVO_PROJECTILE_COUNT * sizeof(Vector2));
            _ability->overall_process = 0.f;

            _ability->position = system->owner_position;
            _ability->fire_rate = SALVO_FIRE_RATE;

            for (size_t i = 0; i <= SALVO_PROJECTILE_COUNT; i++) {
                _ability->projectiles[i].initialized = false;
                _ability->projectiles[i].position = _ability->position;
                _ability->projectiles[i].speed = 1.f;
        ...
        case radiation: {
            _ability->projectiles = (Character2D*)malloc(sizeof(Character2D));
            _ability->position = system->owner_position;
    ...
    for (size_t i = 0; i < system->ability_amount + 1; i++) {
        ...
        switch (_ability->type) {
            ...
            case salvo: {
                ...
                for (size_t j = 0; j < SALVO_PROJECTILE_COUNT; j++) {
                    if (_ability->projectiles[j].initialized) {
                        ...
                        _ability->projectile_process[j] += _ability->projectiles[j].speed * GetFrameTime();
                ...
                for (size_t j = 0; j < SALVO_PROJECTILE_COUNT; j++) {
                    _ability->projectiles[j].rotation = GetRandomValue(0, 360);
                    _ability->projectiles[i].speed = GetRandomValue(0, 100) / 100 + 1;
