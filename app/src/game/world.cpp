...
void create_worldmap_stage(i32 in_map_id, std::string in_displayname, std::string filename, i32 in_stage_level,
  i32 total_spawn_count, f32 spawn_scale, f32 boss_scale, 
  Vector2 in_screen_location, bool is_active) {
  state->worldmap_locations.at(in_map_id) = worldmap_stage(
    in_map_id, in_displayname, filename, in_stage_level, total_spawn_count, spawn_scale, boss_scale, in_screen_location, true, is_active
  );
}
...
bool world_system_initialize(const app_settings* _in_app_settings) {
  ...
  { // WORLD LOCATIONS
    create_worldmap_stage( 0, "Stage 1",  "stage1" , 1, 0,  1, .5f, VECTOR2(   0,    0), false); // main menu background
    create_worldmap_stage( 1, "Stage 2",  "stage2" , 1, 10, 1, .5f, VECTOR2(1024, 1744), true );
    create_worldmap_stage( 2, "Stage 3",  "stage3" , 1, 75, 1, .5f, VECTOR2(1467, 1755), true );
    create_worldmap_stage( 3, "Stage 4",  "stage4" , 1, 75, 1, .5f, VECTOR2(1674, 1727), true );
    create_worldmap_stage( 4, "Stage 5",  "stage5" , 1, 75, 1, .5f, VECTOR2(2179, 1623), true );
    create_worldmap_stage( 5, "Stage 6",  "stage6" , 1, 75, 1, .5f, VECTOR2(1811, 1230), true );
    create_worldmap_stage( 6, "Stage 7",  "stage7" , 1, 75, 1, .5f, VECTOR2(2233, 1240), true );
    create_worldmap_stage( 7, "Stage 8",  "stage8" , 1, 75, 1, .5f, VECTOR2(2658, 1202), true );
    create_worldmap_stage( 8, "Stage 9",  "stage9" , 1, 75, 1, .5f, VECTOR2(3009, 1511), true );
    create_worldmap_stage( 9, "Stage 10", "stage10", 1, 75, 1, .5f, VECTOR2(2767, 1972), true );
    create_worldmap_stage(10, "Stage 11", "stage11", 1, 75, 1, .5f, VECTOR2(3188, 1415), true );
    create_worldmap_stage(11, "Stage 12", "stage12", 1, 75, 1, .5f, VECTOR2(3449, 1773), true );
    create_worldmap_stage(12, "Stage 13", "stage13", 1, 75, 1, .5f, VECTOR2(3073, 1146), true );
    create_worldmap_stage(13, "Stage 14", "stage14", 1, 75, 1, .5f, VECTOR2(3574, 1291), true );
    create_worldmap_stage(14, "Stage 15", "stage15", 1, 75, 1, .5f, VECTOR2(3462,  773), true );
    create_worldmap_stage(15, "Stage 16", "stage16", 1, 75, 1, .5f, VECTOR2(2661,  951), true );
    create_worldmap_stage(16, "Stage 17", "stage17", 1, 75, 1, .5f, VECTOR2(3548,  974), true );
    create_worldmap_stage(17, "Stage 18", "stage18", 1, 75, 1, .5f, VECTOR2(2400,  596), true );
    create_worldmap_stage(18, "Stage 19", "stage19", 1, 75, 1, .5f, VECTOR2(3290,  369), true );
    create_worldmap_stage(19, "Stage 20", "stage20", 1, 75, 1, .5f, VECTOR2(3449,  224), true );
    create_worldmap_stage(20, "Stage 21", "stage21", 1, 75, 1, .5f, VECTOR2(1977,  385), true );
    create_worldmap_stage(21, "Stage 22", "stage22", 1, 75, 1, .5f, VECTOR2(1661,  410), true );
  }
